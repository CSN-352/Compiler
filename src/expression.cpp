...
Expression *create_postfix_expression(Expression *x, Terminal *op)
{
    ...
    if (op->name == "INC_OP")
        P->name = "POSTFIX EXPRESSION INC OP";
    else
        P->name = "POSTFIX EXPRESSION DEC OP";
...
Expression *create_unary_expression(Expression *x, Terminal *op)
{
    ...
    if (op->name == "INC_OP")
        U->name = "UNARY EXPRESSION INC_OP";
    else if (op->name == "DEC_OP")
        U->name = "UNARY EXPRESSION DEC_OP";
    else
        U->name = "UNARY EXPRESSION SIZEOF";
...
Expression *create_unary_expression_cast(Expression *x, Terminal *op)
{
    ...
    else if (op->name == "NOT")
    {
        ...
        {
            if (x->true_list.find(i) == x->true_list.end() && x->false_list.find(i) == x->false_list.end())
                U->code.erase(remove(U->code.begin(), U->code.end(), i), U->code.end()); // TAC
        }
        ...
        {
            if (x->jump_true_list.find(i) == x->jump_true_list.end() && x->jump_false_list.find(i) == x->jump_false_list.end())
                U->jump_code.erase(remove(U->jump_code.begin(), U->jump_code.end(), i), U->jump_code.end()); // TAC
        }
...
Expression *create_relational_expression(Expression *left, Terminal *op, Expression *right)
{
    ...
    if ((lt.isIntorFloat() && rt.isIntorFloat()))
    {
        ...
        if (lt.type_index > rt.type_index)
        {
            ...
            backpatch(left->jump_false_list, i1->label);                                                               // TAC
            backpatch(right->jump_false_list, i1->label);                                                              // TAC
            TACInstruction *i2 = emit(TACOperator(op->name == "LESS" ? TAC_OPERATOR_LT : op->name == "LE_OP" ? TAC_OPERATOR_LE
                                                                                     : op->name == "GREATER" ? TAC_OPERATOR_GT
                                                                                                             : TAC_OPERATOR_GE),
                                      new_empty_var(), left->result, t1, 2);                                                 // TAC
            TACInstruction *i3 = emit(TACOperator(TAC_OPERATOR_NOP), new_empty_var(), new_empty_var(), new_empty_var(), 1);  // TAC
        ...
        {
            R->result = new_temp_var(); // TAC
            TACInstruction *i1 = emit(TACOperator(op->name == "LESS" ? TAC_OPERATOR_LT : op->name == "LE_OP" ? TAC_OPERATOR_LE
                                                                                     : op->name == "GREATER" ? TAC_OPERATOR_GT
                                                                                                             : TAC_OPERATOR_GE),
                                      new_empty_var(), left->result, right->result, 2);                                      // TAC
            TACInstruction *i2 = emit(TACOperator(TAC_OPERATOR_NOP), new_empty_var(), new_empty_var(), new_empty_var(), 1);  // TAC
            ...
            TACInstruction *i6 = emit(TACOperator(op->name == "LESS" ? TAC_OPERATOR_LT : op->name == "LE_OP" ? TAC_OPERATOR_LE
                                                                                     : op->name == "GREATER" ? TAC_OPERATOR_GT
                                                                                                             : TAC_OPERATOR_GE),
                                      new_empty_var(), left->result, right->result, 2);                                     // TAC if goto
            TACInstruction *i7 = emit(TACOperator(TAC_OPERATOR_NOP), new_empty_var(), new_empty_var(), new_empty_var(), 1); // TAC goto
            TACInstruction *i6_ = emit(TACOperator(op->name == "LESS" ? TAC_OPERATOR_LT : op->name == "LE_OP" ? TAC_OPERATOR_LE
                                                                                      : op->name == "GREATER" ? TAC_OPERATOR_GT
                                                                                                              : TAC_OPERATOR_GE),
                                       new_empty_var(), left->result, right->result, 2);                                     // TAC if goto
        ...
        else
        {
            ...
            TACInstruction *i2 = emit(TACOperator(op->name == "LESS" ? TAC_OPERATOR_LT : op->name == "LE_OP" ? TAC_OPERATOR_LE
                                                                                     : op->name == "GREATER" ? TAC_OPERATOR_GT
                                                                                                             : TAC_OPERATOR_GE),
                                      new_empty_var(), t1, right->result, 2);                                                // TAC
            TACInstruction *i3 = emit(TACOperator(TAC_OPERATOR_NOP), new_empty_var(), new_empty_var(), new_empty_var(), 1);  // TAC
            TACInstruction *i4 = emit(TACOperator(TAC_OPERATOR_NOP), R->result, new_constant("1"), new_empty_var(), 0);      // TAC
            TACInstruction *i5 = emit(TACOperator(TAC_OPERATOR_NOP), new_empty_var(), new_empty_var(), new_empty_var(), 1);  // TAC
            TACInstruction *i5_ = emit(TACOperator(TAC_OPERATOR_NOP), new_empty_var(), new_empty_var(), new_empty_var(), 1); // TAC
    ...
    if (op->name == "LOGICAL_AND")
    {
        if (lt.isIntorFloat() && rt.isIntorFloat())
        {
            ...
            if (lt.isUnsigned() != rt.isUnsigned())
            {
                ...
                {
                    if (left->true_list.find(i) == left->true_list.end() && left->false_list.find(i) == left->false_list.end())
                        L->code.erase(remove(L->code.begin(), L->code.end(), i), L->code.end()); // TAC
                }
                for (auto i : right->jump_true_list)
                {
                    if (right->true_list.find(i) == right->true_list.end() && right->false_list.find(i) == right->false_list.end())
                        L->code.erase(remove(L->code.begin(), L->code.end(), i), L->code.end()); // TAC
                }
                ...
                {
                    if (left->jump_true_list.find(i) == left->jump_true_list.end() && left->jump_false_list.find(i) == left->jump_false_list.end())
                        L->jump_code.erase(remove(L->jump_code.begin(), L->jump_code.end(), i), L->jump_code.end()); // TAC
                }
                for (auto i : right->true_list)
                {
                    if (right->jump_true_list.find(i) == right->jump_true_list.end() && right->jump_false_list.find(i) == right->jump_false_list.end())
                        L->jump_code.erase(remove(L->jump_code.begin(), L->jump_code.end(), i), L->jump_code.end()); // TAC
                }
...
Expression *create_conditional_expression(Expression *condition, Expression *true_expr, Expression *false_expr)
{
    ...
    else if (tt == ft)
    {
        ...
        {
            if (condition->true_list.find(i) == condition->true_list.end() && condition->false_list.find(i) == condition->false_list.end())
                C->code.erase(remove(C->code.begin(), C->code.end(), i), C->code.end()); // TAC
        }
        ...
        {
            if (condition->jump_true_list.find(i) == condition->jump_true_list.end() && condition->jump_false_list.find(i) == condition->jump_false_list.end())
                C->jump_code.erase(remove(C->jump_code.begin(), C->jump_code.end(), i), C->jump_code.end()); // TAC
        }
...
Expression *create_assignment_expression(Expression *left, Terminal *op, Expression *right)
{
    ...
    else if (op->name == "AND_ASSIGN" || op->name == "OR_ASSIGN" || op->name == "XOR_ASSIGN")
    {
        ...
        if (lt != rt)
        {
            ...
            backpatch(right->jump_true_list, i1->label);                                                               // TAC
            backpatch(right->jump_false_list, i1->label);                                                              // TAC
            TACInstruction *i2 = emit(TACOperator(op->name == "AND_ASSIGN" ? TAC_OPERATOR_BIT_AND : op->name == "OR_ASSIGN" ? TAC_OPERATOR_BIT_OR
                                                                                                                            : TAC_OPERATOR_BIT_XOR),
                                      left->result, left->result, t1, 0); // TAC
            A->code.push_back(i1);                                        // TAC
            A->code.push_back(i2);                                        // TAC
        ...
        {
            TACInstruction *i1 = emit(TACOperator(op->name == "AND_ASSIGN" ? TAC_OPERATOR_BIT_AND : op->name == "OR_ASSIGN" ? TAC_OPERATOR_BIT_OR
                                                                                                                            : TAC_OPERATOR_BIT_XOR),
                                      left->result, left->result, right->result, 0); // TAC
            backpatch(left->next_list, i1->label);                                   // TAC
            backpatch(left->jump_next_list, i1->label);                              // TAC
            backpatch(right->next_list, i1->label);                                  // TAC
            backpatch(right->jump_next_list, i1->label);                             // TAC
...
{ // top level expression - corresponds to non terminal expression in grammar
